...
from analysis.metrics import (
    load_clean_data, jobs_per_category, skills_coverage, top_skills, top_companies
)

st.set_page_config(page_title="Remote Job Market Intelligence", layout="wide")

@st.cache_data
def get_data():
    return load_clean_data(PROJECT_ROOT / "data/processed/wwr_jobs_clean.csv")
...
all_categories = sorted(df["category_final"].dropna().unique())  # options from FULL df, see note below
...
col1.metric("Jobs Shown", f"{len(filtered)} / {len(df)}")
col2.metric("Categories", filtered["category_final"].nunique())
col3.metric("Hot Jobs", int(filtered["is_hot"].sum()))
col4.metric(
    "Skill-Tag Coverage",
    f"{coverage['pct_with_skills']}%",
    help=f"{coverage['jobs_with_skills']} of {coverage['total_jobs']} shown listings include structured skill tags"
)
...
fig.update_layout(xaxis_tickangle=-40, height=450, margin=dict(b=120))
...
with col_left:
    ...
    if coverage["jobs_with_skills"] == 0:
        st.info("None of the currently filtered jobs include structured skill tags.")
    else:
        st.caption(
            f"Based on {coverage['jobs_with_skills']} of {coverage['total_jobs']} shown listings "
            f"({coverage['pct_with_skills']}%) that include structured skill tags. "
            "Generic terms (Full Time, Engineer, Developer, Full Stack Dev) excluded."
        )
        skills_df = top_skills(filtered, n=12, exclude_noise=True)
...
with col_right:
    ...
    companies_df = top_companies(filtered, n=12)
    fig_companies = px.bar(companies_df, x="count", y="company", orientation="h")
    fig_companies.update_layout(yaxis=dict(categoryorder="total ascending"), height=450)
    st.plotly_chart(fig_companies, use_container_width=True)

st.divider()

st.subheader(f"Job Listings ({len(filtered)} of {len(df)})")

display_df = filtered[["title", "company", "category_final", "job_type", "posted_date", "url"]].copy()
display_df = display_df.rename(columns={"category_final": "category"})
display_df["posted_date"] = display_df["posted_date"].dt.strftime("%b %d, %Y")
